...
async function fetchData() {
  return await filedata("./public/Wordnetsynonyms.json");
}
          ...
          {currentItems?.map((item, i) => (
            ...
            >
              ...
              <div className="text-lg mb-4 text-center">
                <span className="font-semibold text-green-300">Synonyms:</span>{" "}
                <div className="flex items-center justify-center gap-[20px] flex-wrap mt-[20px]">
                  {item.Synonyms.split(";").map((synonym, i) => (
                    <li key={i} className="text-white">
                      {synonym.split("|").map((subSynonym, j) => (
                        <span key={j}>{subSynonym}</span>
                      ))}
                    </li>
                  ))}
                </div>
